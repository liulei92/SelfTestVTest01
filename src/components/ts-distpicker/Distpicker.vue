...
export default {
  ...
  created () {
    // 获取行政区域标准数据并存入sessionStorage中
    let getCommonDistData = () => {
      let data = sessionStorage.getItem('commonDistData')

      if (data !== '' && data !== undefined && data !== null) return false

      this.ajax.fetch({
        url: this.apis.public.area,
        successFunc: res => {
          console.log('获取行政区域标准数据成功！' + new Date())
          sessionStorage.removeItem('commonDistData')
          sessionStorage.setItem('commonDistData', JSON.stringify(res))
        },
        errorFunc: res => {
          // this.toast.showError(res.info)
          console.log(res)
        }
      })
    }
  ...
  watch: {
    ...
    currentArea (value) {
      // 对区做特殊处理，原因有些不同市的区名称相同
      let data = {
        code: undefined,
        value: value
      }
      for (const key in this.areas) {
        if (this.areas[key] === value) {
          data.code = key
        }
      }
      this.$emit('area', data)
      // this.$emit('area', this.setData(value, this.currentCity)) 用区的时候会有问题，暂时用上面代码代替，v-distpicker版本是1.0.20
  ...
  methods: {
    ...
    emit (name) {
      ...
      if (!this.onlyProvince || this.hideArea) {
        ...
        for (const key in this.areas) {
          if (this.areas[key] === this.currentArea) {
            area.code = key
          }
        }
        this.$set(data, 'area', area)
